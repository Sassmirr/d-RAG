...
import { auth, googleProvider } from "@/firebase"; // make sure this is configured
import {
...
interface LoginScreenProps {
  onLogin: (user: { name: string; email: string; uid: string, photoURL?: string }) => void;
}
...
export const LoginScreen = ({ onLogin }: LoginScreenProps) => {
 ...
 const handleGoogleLogin = async () => {
  ...
  try {
    ...
    const { displayName, email: userEmail, uid, photoURL } = result.user;

    onLogin({
      name: displayName || userEmail.split("@")[0],
      email: userEmail,
      uid,
      photoURL: photoURL || "", // ensure fallback value
    });
  } catch (err: any) {
    toast({
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-card border-border shadow-elevated">
        <CardHeader className="text-center space-y-4">
          ...
          <div>
            <CardTitle className="text-2xl text-foreground">Welcome to d-RAG</CardTitle>
